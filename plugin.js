...
function print(path, options$$1, print) {
    ...
    switch (node.type) {
        ...
        case 'AwaitBlock': {
            ...
            const hasThenBlock = node.then.children.length !== 0 && !node.then.children.every(isEmptyNode);
            ...
            if (!hasPendingBlock && hasThenBlock) {
                ...
                if (hasThenBlock) {
                    block.push(group(concat(['{:then', node.value ? ' ' + node.value : '', '}'])), indent(path.call(print, 'then')));
                }
            }
            if (hasCatchBlock) {
                block.push(group(concat(['{:catch', node.error ? ' ' + node.error : '', '}'])), indent(path.call(print, 'catch')));
            }
            block.push('{/await}');
